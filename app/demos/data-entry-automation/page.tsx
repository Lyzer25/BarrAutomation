...
function ROICalculator({
  ...
  const calculations = useMemo(() => {
    ...
    const coveragePct =
      recordsPerDayNum > 0 ? Math.min(100, Math.round((humanTotalCapacity / recordsPerDayNum) * 100)) : 0
    ...
    const humanProcessingTimeHoursPerPerson =
      numEmployees > 0 ? humanProcessingTimeHoursTotal / numEmployees : humanProcessingTimeHoursTotal
    ...
    <div className="space-y-6">
      ...
      <div className="mt-3 md:mt-0">
        <Label htmlFor="data-entry-employees" className="text-sm text-muted-foreground flex items-center gap-1">
          ...
          <Tooltip>
            ...
            <TooltipContent>
              Set how many employees currently perform data-entry. Use 0 to model an AI-only scenario.
            </TooltipContent>
          </Tooltip>
        </Label>
        <Input
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <Card className="p-4 bg-white/5">
          ...
          <div
            className={`text-2xl font-bold ${calculations.netMonthlySavings > 0 ? "text-green-400" : "text-red-400"}`}
          >
            $
            {Math.abs(calculations.netMonthlySavings)
              .toFixed(0)
              .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
          </div>
        ...
        <Card className="p-4 bg-white/5">
          ...
          <div className="text-xs text-muted-foreground mt-1">Including $5K setup</div>
        </Card>

        <Card className="p-4 bg-white/5">
          <div className="text-xs text-muted-foreground mb-2">Payback Period</div>
          <div
            className={`text-2xl font-bold ${calculations.paybackPeriod > 0 && calculations.paybackPeriod < 12 ? "text-green-400" : calculations.paybackPeriod >= 12 ? "text-yellow-400" : "text-gray-400"}`}
          >
            {calculations.paybackPeriod > 0 ? `${calculations.paybackPeriod.toFixed(1)} mo` : "N/A"}
          </div>
          <div className="text-xs text-muted-foreground mt-1">To recover setup cost</div>
        </Card>

        <Card className="p-4 bg-white/5">
          <div className="text-xs text-muted-foreground mb-2">Number of data entry employees</div>
          <div className="text-2xl font-bold text-red-400">{calculations.numEmployees}</div>
          <div className="text-xs text-muted-foreground mt-1">vs 1 AI system</div>
        </Card>
      </div>

      {/* First Year Summary */}
      <Card className="p-4 bg-gradient-to-r from-green-900/20 to-red-900/20 border-green-500/20">
        <div className="flex justify-between items-center">
          <div>
            <div className="text-sm text-muted-foreground">First Year Total Savings</div>
            <div
              className={`text-3xl font-bold ${calculations.firstYearSavings > 0 ? "text-green-400" : "text-red-400"}`}
            >
              $
              {Math.abs(calculations.firstYearSavings)
                .toFixed(0)
                .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
            </div>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Card className="p-4 bg-white/5">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            ...
            <div
              className={`text-xs font-semibold ${calculations.coveragePct >= 100 ? "text-green-400" : calculations.coveragePct >= 50 ? "text-yellow-400" : "text-red-400"}`}
            >
            ...
            <div className="text-xs">
              {calculations.humanProcessingTimeHoursTotal.toFixed(1)} hrs/day total •{" "}
              {calculations.humanProcessingTimeHoursPerPerson.toFixed(1)} hrs/day per person
            </div>
          </div>
        </Card>
      ...
      <p className="text-xs text-muted-foreground text-center">
        AI service requires a one-time $5,000 setup fee plus $1,500/month for unlimited processing. Most businesses see
      ...
      <main className="container mx-auto px-4 py-10">
        ...
        <header className="flex items-start justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-2 text-sm text-subtle-gray">
              Watch AI convert messy inputs into clean, mapped records — a simple, guided demo.
            </p>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="text-xs text-muted-foreground">
              Runs:{" "}
              <span aria-live="polite" className="font-mono ml-1">
                {runs}
              </span>
            </div>
            <div className="text-xs text-muted-foreground">
        ...
        <section className="mb-6">
          <div
            ...
            className="inline-flex rounded-xl overflow-hidden border border-white/10"
          >
            {USE_CASE_KEYS.map((k) => {
              const s = SAMPLES[k]
        ...
        <section className="grid md:grid-cols-2 gap-6 mb-6">
          <Card className="p-4" aria-labelledby="sample-heading">
            <div className="flex items-center justify-between mb-2">
              <h4 id="sample-heading" className="font-medium">
                Sample Data
              </h4>
            ...
            <div className="mt-3 flex gap-3">
              <Button onClick={insertSample} aria-label="Insert sample data">
                Insert Sample
              </Button>
          ...
          <Card className="p-4" aria-labelledby="results-heading">
            <div className="flex items-center justify-between mb-2">
              <h4 id="results-heading" className="font-medium">
                Extraction Results
              </h4>
            ...
            {loading ? (
              ...
              <>
                ...
                <ul className="mt-2 text-sm text-muted-foreground space-y-1">
                  {mapping.map((m, i) => (
                    <li key={i}>
                      <strong>{m.from}</strong> → <span className="font-mono">{m.to}</span>
                    </li>
                  ))}
                </ul>
              </>
        ...
        <section className="mb-6">
          <p className="text-sm text-muted-foreground">
            This demo runs entirely in your browser using mocked extractors and mappers — no external networks are
            called.
          </p>
        </section>
        ...
        <section className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 p-4 rounded border border-white/6">
          <p className="text-sm text-muted-foreground">
            Ready to map this to your stack? We tailor extractors, validators, and connectors to your process.
          </p>
