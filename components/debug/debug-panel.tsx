...
export default function DebugPanel({ className }: DebugPanelProps) {
  ...
  const getFilteredEvents = () => {
    ...
    if (filterLevel !== "all") {
      filtered = filtered.filter((event: any) => {
        ...
        if (filterLevel === "automation")
          return event.type.includes("AUTOMATION") || event.type.includes("STATUS") || event.type.includes("DASHBOARD")
        return true
      })
    }
  ...
  const getEventTypeColor = (type: string) => {
    ...
    if (type.includes("SSE")) return "bg-red-500"
          ...
          <div className="w-64 border-r border-gray-700 bg-gray-800 flex flex-col">
            <div className="p-4 border-b border-gray-700">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={handleRefresh} className="flex-1 bg-transparent">
                  🔄 Refresh
                </Button>
                <Button
                  variant="outline"
            ...
            <nav className="flex-1 p-2">
              {[
                ...
                { id: "state", label: "State", icon: "🔄", count: debugData?.stateHistory?.length || 0 },
                {
                  ...
                  label: "Performance",
                  icon: "⚡",
                  count: debugData?.performance?.memoryUsage?.length || 0,
                },
                  ...
                  <ScrollArea className="h-full p-4">
                    <div className="space-y-2">
                      {getFilteredEvents().map((event: any) => (
                        <div key={event.id} className="bg-gray-800 border border-gray-700 rounded p-3 text-sm">
                    ...
                    <div className="space-y-2">
                      {debugData?.networkCalls?.slice(-50).map((call: any, index: number) => (
                        <div key={index} className="bg-gray-800 border border-gray-700 rounded p-3 text-sm">
                          <div className="flex items-center gap-2 mb-2">
                            <Badge className={call.success ? "bg-green-500" : "bg-red-500"}>{call.method}</Badge>
                            <span className="text-gray-400 text-xs">
                              {call.status} • {call.duration}ms
                            </span>
                          </div>
                          <div className="text-gray-300 text-xs mb-1">{call.url}</div>
                          <div className="text-gray-400 text-xs">{formatTimestamp(call.timestamp)}</div>
                        </div>
                      ))}
                    </div>
                  </ScrollArea>
                      ...
                      {debugData?.stateHistory?.slice(-30).map((state: any, index: number) => (
                        <div key={index} className="bg-gray-800 border border-gray-700 rounded p-3 text-sm">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-gray-400 text-xs">{formatTimestamp(state.timestamp)}</span>
                          </div>
                          <div className="text-gray-300 text-xs mb-1">Reason: {state.reason}</div>
                    ...
                    <div className="space-y-2">
                      {debugData?.events?.slice(-20).map((event: any) => (
                        <div key={event.id} className="flex items-center gap-4 py-2 border-b border-gray-700">
                          <div className="text-xs text-gray-400 w-20">{formatTimestamp(event.timestamp)}</div>
                  ...
                  <div className="p-4 space-y-4">
                    <div className="bg-gray-800 border border-gray-700 rounded p-4">
                      ...
                      <div className="space-y-3">
                        <Button onClick={() => debugStore.exportReport()} className="w-full justify-start">
